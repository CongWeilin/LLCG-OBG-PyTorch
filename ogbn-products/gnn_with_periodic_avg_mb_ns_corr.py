...
def print_local_step_size(args):
    epochs = []
    for i in range(args.epochs):
        epochs.append(args.local_steps*(args.local_ratio**i))
        print('Epoch: ', i, ', epoch size: ', int(epochs[-1]), ', all local steps: ', int(np.cumsum(epochs)[-1]))
...
def main():
    ...
    ##############################################
    # for server training
    server_sampler = NeighborSampler(sparse_tensor_to_edge_indices(data.adj_t), 
    ...
    for run in range(args.runs):
        ...
        for epoch in range(args.epochs):
            ...
            print('Epoch: ', epoch+1, ', Time: ', time.time()-epoch_start_time)
            ...
            if epoch % args.avg_per_num_epoch == 0:
                ...
                server_loss = train(model, data.x, data.y.squeeze(), 
