...
def main():
    ...
    for run in range(args.runs):
        ...
        for epoch in range(1, 1 + args.epochs):
            ...
            for part_id in range(args.num_parts):
                start_t = time.time()
                loss_ = train_partition(part_model, data, 
                                        part_optimizer, part_id, 
                                        args.use_cut_edges)
    ...
    torch.save(torch.tensor(logger.results), 'results_period_avg.pt')
    torch.save(torch.tensor(model_dist), 'model_dist_period_avg.pt')

    torch.save(torch.tensor(timer.local_time),  'local_time_period_avg.pt')
